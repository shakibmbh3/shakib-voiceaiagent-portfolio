...
export const Hero: React.FC = () => {
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
      ...
      <div className="absolute top-0 right-0 w-1/3 h-1/3 bg-emerald-100/50 rounded-full blur-3xl opacity-50 transform translate-x-1/4 -translate-y-1/4" />
      <div className="absolute bottom-0 left-0 w-1/3 h-1/3 bg-emerald-100/50 rounded-full blur-3xl opacity-50 transform -translate-x-1/4 translate-y-1/4" />
      ...
      <div className="container mx-auto px-6 relative z-10 text-center max-w-4xl">
        ...
        <p className="text-lg md:text-xl text-slate-600 mb-8 max-w-2xl mx-auto leading-relaxed animate-fade-in-up [animation-delay:200ms]">
          I build custom AI voice agents that handle real business calls, 
          automate follow-ups, and sync directly with your CRM.
        </p>

        <div className="flex flex-wrap justify-center gap-4 mb-10 text-sm md:text-base text-slate-700 animate-fade-in-up [animation-delay:300ms]">
          {[
            "Production-ready AI voice agents",
            "Call flows, APIs, CRM integrations",
            "Fully custom. No templates"
          ].map((point, i) => (
            <div key={i} className="flex items-center gap-2 bg-white/60 backdrop-blur-sm px-4 py-2 rounded-full border border-emerald-100 shadow-sm">
